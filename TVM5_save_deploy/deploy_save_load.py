# -*- coding: utf-8 -*-
"""
Created on Tue Sep 20 20:23:09 2022
...
# under the License.
"""
Deploy a Quantized Model on Cuda/ cpu
================================
...
calibration_rec = download_testdata(
    "http://data.mxnet.io.s3-website-us-west-1.amazonaws.com/data/val_256_q90.rec",
    "val_256_q90.rec", module='data'
)
# calibration_rec =download_testdata("C:\\Users\\myname\\.tvm_test_data\\data\\val_256_q90.rec")
...
if __name__ == "__main__":
    # main()
    mod_fp32, params = get_model()
    mod = quantize(mod_fp32, params, data_aware=True)
    # run_inference(mod)
    model1 = relay.create_executor("vm", mod_fp32, dev, target).evaluate()
    model2 = relay.create_executor("vm", mod, dev, target).evaluate()
    # with open("./quant_model/tflite_model.onnx","wb")as f:
    #     f.write(model1)
    val_data, batch_fn = get_val_data()
    for i, batch in enumerate(val_data):
        data, label = batch_fn(batch)
        ...
        scores1 = softmax(prediction1)
        scores1 = np.squeeze(scores1)
        ranks1 = np.argsort(scores1)[::-1] #逆序排列，输出索引是从大到小的
        idx=ranks1[0]
        top1_tvm1 = np.argmax(prediction1[0])
        # print("class='%s' with probability=%f" % (idx, scores[idx]))
        for rank in ranks1[0:3]:
            print("fp32-class='%s' with probability=%f" % (rank, scores1[rank]))
        #model2
        prediction2 = model2(data)
        prediction2 = prediction2.numpy()
        scores2 = softmax(prediction2)
        scores2 = np.squeeze(scores2)
        ranks2 = np.argsort(scores2)[::-1] #逆序排列，输出索引是从大到小的
        idx=ranks2[0]
        top1_tvm2 = np.argmax(prediction2[0])
        # print("class='%s' with probability=%f" % (idx, scores[idx]))
        for rank in ranks2[0:3]:
            print("quant-class='%s' with probability=%f" % (rank, scores2[rank]))
        # yield {"data": data}
        if i >3:  # only run inference on a few samples in this tutorial
            break
    
# fp32预测
# with tvm.transform.PassContext(opt_level=3):
#     lib = relay.build(mod_fp32, target=target, params=params)

# dev = tvm.device(str(target), 0)
# module_32 = graph_executor.GraphModule(lib["default"](dev))
# 展示一张图
    import cv2
    data1=np.squeeze(data)
    img=np.transpose(data1,(1,2,0))
    img=cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    cv2.imshow('img',img) #序号980对应label：火山
    cv2.waitKey(0) 
